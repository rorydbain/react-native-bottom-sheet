...
const BottomSheetComponent = forwardRef<BottomSheet, BottomSheetProps>(
    function BottomSheet(props, ref) {
        ...
        const animatedSheetHeight = useDerivedValue(
            () => animatedContainerHeight.value - (animatedHighestSnapPoint.value ?? 0),
        ...
        const getEvaluatedPosition = useWorkletCallback(
            function getEvaluatedPosition(source: ANIMATION_SOURCE): number {
                'worklet';
                ...
                if (
                    ...
                    return Math.max(0, (highestSnapPoint ?? 0) - keyboardHeightInContainer);
                }
                ...
                if (!isAnimatedOnMount.value) {
                    return _providedIndex === -1
                        ? (animatedClosedPosition.value ?? 0)
                        : snapPoints[_providedIndex] ?? 0;
                }

                /**
                 * return the current index position.
                 */
                return snapPoints[currentIndex] ?? 0;
            },
            [
            ...
            function evaluatePosition(
                ...
                if (animatedAnimationState.value === ANIMATION_STATE.RUNNING) {
                    ...
                    if (animatedNextPositionIndex.value !== animatedCurrentIndex.value) {
                        animateToPosition(
                            animatedSnapPoints.value[animatedNextPositionIndex.value] ?? 0,
                            source,
                            undefined,
                            animationConfigs
                        );
